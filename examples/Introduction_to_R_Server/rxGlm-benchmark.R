...
csvFile <- "Datasets/airOT201201.csv"
...
xdfFile <- "Datasets/AirOnTime2012.xdf"
...
timing_R <- data.frame(R = "CRAN R glm()", nrows = nrow_R, time = timing_R)
timing_R

testFunMRS <- function(nrows){
  xdf_subset <- tempfile(fileext = ".xdf")
  on.exit(file.remove(xdf_subset))
  rxDataStep(xdf, outFile = xdf_subset, numRows = nrows, overwrite = TRUE, reportProgress = 1)
  rxSetComputeContext(RxLocalParallel())
  system.time(
    model2 <- rxLogit(ArrDel15 ~ Origin + DayOfWeek + DepTime,
                      data = xdf_subset,
...
timing_MRS <- data.frame(R = "R Server rxLogit()", nrows = nrow_MRS, time = timing_MRS)
timing_MRS
