...
# Go to http://finance.yahoo.com/q/hp?s=IBM+Historical+Prices and copy the link 
# to the table. Then read the data directly from the URL into an R data frame.	
...
# Having taken the trouble to fetch the data from the web we will then 
# show how to write it to disk and read it back into a different data frame.	
...
# Here are two ways to add a new variable, Volatility, to a data frame, 
# The first uses "$" to index into the data frame, The second uses the 
# within function.
...
# We show two was to prune the data frame so that it only include prices after 
# January 1, 2000. Note that both methods use row, column indexing into 
# the data frame IBM.stock[row,column]. The first thing we do is check 
# to see what data types the varables are. Noticing that Date is a factor, 
# we will make it a date as we build a new data frame.	
...
# The second method uses the which() function. First, we build a 
# simple example to show how which() works.	
xx <- 1:10	
which(xx > 5)	
...
# First we will create new year and month variables by extracting 
# the relevant information from the Date variable	

# Add variable Month to data frame	
IBM.stock.2000$Month <- substr(IBM.stock.2000$Date, 6, 7)
# Add variable Year to the data frame	
IBM.stock.2000$Year  <- substr(IBM.stock.2000$Date,1,4)			
head(IBM.stock.2000)	

# Make a new data frame to hold the aggregated monthly prices.	
# The dot in the formula stands for everything.
IBM.stock.month <- aggregate(.~Month+Year,data=IBM.stock.2000,mean) 
head(IBM.stock.month)	

# Make a date variable in IBM.stock.month and assign everything to 
# the first of the month	
IBM.stock.month$Date <- as.Date(
    paste(IBM.stock.month$Year,IBM.stock.month$Month,'01',sep='-'))	
head(IBM.stock.month)	

# Now sort the data frame to get the latest data first	
IBM.stock.month <- IBM.stock.month[with(
    IBM.stock.month,order(-as.integer(Year),Month)),]	
head(IBM.stock.month)	


### Merge data	

# We will merge the the data frame containing the aggregated monthly 
# stock prices from the year 2000 to the present with a new data frame 
# containing dividend data. First we get the dividend data.	
...
# Next we write a function to Create a column for the merge picking 
# the last dividend dispersed before the merge.	
...
# Finally, let's look at using the reshape function to make 
# "wide" and "long" versions of the data set.	

IBM.wide <- reshape(IBM[, c("Year", "Month", "Close")], idvar = "Year",
...
# But for the past year or so "state-of-the-art"" data wrangling with R 
# is accomplished through Hadley Wichham's dplyr package. Let's look at 
# Hadly's tutorial in his online vignette.	
