...
# In this section we will show off some R features for exploring data 
# using plots from various contributed packages.	

### The Weather Data	

# We will use the weather data set that is included in the rattle package. 
# First we fetch the data from the package and then write it to disk in 
# the working directory. After that, we can read the data directly from disk.	
...
nm <- names(weather)	
ind <- which(nm == "Sunshine" | nm == "WindGustSpeed" |
             nm=="WindSpeed9am"|nm=="WindSpeed3pm")	
...
# We are eventually going to look at some machine learning algorithms 
# with this data set. Our "target" variable for predictions will be 
# the binary variable, RainTomorrow. So let's produce a bar plot to have 
# a look at it.  The 'gplots' package provides the 'barplot2' function. 
# First, we generate the summary data for plotting.	
ds <- summary(na.omit(weather$RainTomorrow))	
# Plot the data.	
bp <- barplot2(ds, beside = TRUE, ylab = "Frequency",
               xlab="RainTomorrow", ylim=c(0, max(ds)+15), col="blue")	
...
# make the columns of weather available in the global environmentattach(weather)						
jhist <- function(var){	
            hs <- hist(var,                         
                  # title with variable name	
				  main=substitute(paste("Distribution of ",var)),  
	        ...
	        col="grey90",					# color of histogram	
                    ...
                    breaks = "fd",          # use the Freedman-Diaconis algorithm 
                                            # for breakpoints	
...
# Next, we will xonstructa Panel Histogram to look at pairwise correlations. 
# The code for this example comes from page 124 of the book: 
# Data Mining with R and Rattle http://amzn.to/1qzExMc	
...
### Correlations	
# The 'ellipse' package provides the 'plotcorr' function. Correlations work 
# for numeric variables only.	

# to get bick out the numeric variables from weather you can type 
# the names in by hand	
...
nvars <- names(weather[,sapply(weather,is.numeric)==TRUE])	
nvars <- nvars[-17]	
...
# Finally, we will look at correlations among missing values. 
# na.omit removes rows contianing NAs, The row numbers of the cases form 
# the "na.action" attribute of the result, of class "omit". So na.rows 
# below contains the row numbers of the missing data points.
