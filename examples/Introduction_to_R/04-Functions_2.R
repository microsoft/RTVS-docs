...
# Functions have formals(), body() and a parent environment(). 
# These are functions that are helpful for looking at other functions.	
# formals() returns the formal arguments of a function.	

formals(rnorm)    	# returns the formal arguments of a function	

# body() returns thd body of a function. For rnorm(), the body contains.
# External() which is an internal function to call C/C++ code.	
...
# For a great discussin or Environments in R see the Environments chapter of 
...
# Functions in R are ordinary objects. You don't have to assign them a name 
# they can be anonymous. For example:	
...
# R uses lexical scoping: The binding of symbols to values depends on what 
# was available in the function's environment at the time the function 
# was created. This is called the enclosing environment.
x <- 5	
f <- function(){	
	 y <- 10	
	 x + y	
...
# If a variable is not in the current environment R looks in 
# the parent environment. Use search() to see the hierarchy of environments 
# R will search.	
...
# To make something internal to a function available in 
# the parent environment use the <<- asignment operator. 
...
# A ragged array is an array (vector, matrix, etc.) organized into groups 
# by a factor variable where some of the levels of the factor may have 
# zero entries.   	
