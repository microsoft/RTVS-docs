...
# In this script we will look at Kmeans and hierarchical clustering using 
# the weather data.	
set.seed(42)
...
# Note that in the plot: "left" variable is on the y axis and the "under" 
# variable is on the x axis	

# Next, we determine number of clusters by looking for a "knee" in the curve, 
# i.e. the largest drop in the within sum of squares in the plot below	
...
# The plotcluster function in the fpc package. There are parameters 
# to distinguisH classes by ten available projection methods, including 
# classical discriminant coordinates, methods to project differences in mean 
# and covariance structure, asymmetric methods (separation of a homogeneous 
# class from a heterogeneous one), local neighborhood-based methods and 
# methods based on robust covariance matrices. One-dimensional data is 
# plotted against the cluster number. 	

kmPlot <- function(data,nclust=3,...)	
		{	
			km <- kmeans(x=data, centers=nclust)     	
            # plot fcn from fpc package	
            plotcluster(x = data, clvecd = km$cluster, ...)
            title(main=paste(nclust, " clusters"))	
            # Compute cluster stats with fcn from fpc package	
            cluster.stats(dist(data), km$cluster)
            return(km)	
		}	
...
# The following function to produce a hierarchical correlation plot. 
# It follows code on page 135 of Data Mining with Rattle and R.	
...
# varclus(), in the Hmisc package, does a hierarchical cluster analysis on 
# variables, using the Hoeffding D statistic, squared Pearson or Spearman 
# (the default) correlations, or proportion of observations for which two 
# variables are both positive as similarity measures. The clustering is 
# done by hclust(). 	
